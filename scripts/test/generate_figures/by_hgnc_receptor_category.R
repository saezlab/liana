...
intercell_receptor_category_rm_complex <- left_join(intercell_rm_complex,
                                                    hgnc_receptor_categories,
                                                    by = c("target" = "uniprot"))

add_omnipath_rm_complex <- intercell_receptor_category_rm_complex %>% 
  group_by(category) %>% 
  summarise(n = n()) %>%
  dplyr::mutate(sources = "Omnipath")
data_w_omnipath_rm_complex <- intercell_receptor_category_rm_complex %>% 
  tidyr::separate_rows(sources, sep = ";") %>%
  dplyr::filter(sources %in% omni_list) %>% 
  group_by(sources, category) %>%
  summarise(n = n()) %>%

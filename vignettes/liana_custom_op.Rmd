...
## Make use of Intracellular OmniPath
Here, we will perform an over-representation representation (ORA)
to obtain enriched SignaLink pathways in preferentially ranked ligand-receptor
interactions from LIANA.

### First, let's run LIANA with the OmniPath `intercellular` resource as in the manuscript.
#### RUN liana
```{r liana_wrap, message = FALSE, print = FALSE}
# Run liana with the OmniPath resource
# here we pass it as an external resource, as we need it later
op_resource <- decomplexify(liana_omni)
# liana Wrap
...
```{r load_sig}
# Get MSigDB Genest from OmniPath - takes a while :)

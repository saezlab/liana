...
######### Per Cell Type relative to methods (OmniPath)
...
######### Per Cell Type relative to methods (Default)
# Get top 250 and bind csea states
top250_csea <- top250_resource_tool$Default %>%
    enframe(name = "method", value = "results") %>%
    mutate(results = results %>% map(function(res) res %>%
                                         select(source, target, ligand, receptor))) %>%
    unnest(results) %>%
...
# SIGNALINK
top250_signal <- top250_resource_tool$Default %>%
    enframe(name = "method", value = "results") %>%
    mutate(results = results %>% map(function(res) res %>%
                                         select(source, target, ligand, receptor))) %>%
    unnest(results) %>%
...
##### Per Cell type, Relative to all matched ligrecs to geneset (WRONG)
top250_syms_perCT <- top250_resource_tool$OmniPath %>%
    enframe(name = "method", value = "results") %>%
    mutate(results = results %>% map(function(res) res %>%
                                         select(source, target, ligand, receptor))) %>%
    unnest(results) %>%
    pivot_longer(cols = c(ligand, receptor), names_to = "cat", values_to = "genesymbol") %>%
    pivot_longer(cols = c(source, target), names_to = "cell", values_to = "cell_type") %>%
    select(method, cell_type, genesymbol)

top250_enrich_pathway <-signal_syms %>%
    left_join(top250_syms_perCT) %>%
    group_by(cell_type) %>%
    group_split() %>%
    map(function(met)
        met %>%
...
oce_heat <- top250_enrich_pathway %>%
    select(method, cell_type, pathway, enrichment) %>%
    unite(method, cell_type, col = "mc", sep = "_") %>%
    pivot_wider(id_cols = mc, names_from = pathway, values_from = enrichment) %>%
    mutate_all(~ replace(., is.na(.), 0))

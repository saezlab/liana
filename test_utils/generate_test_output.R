...
resource <- select_resource('Consensus')[[1]] %>%
    decomplexify()
lrs <- union(resource$source_genesymbol, resource$target_genesymbol)
...
rownames(sce) <- sample(lrs, 1000, replace = FALSE)
...
cell.adj <- pipe_out %>%
    ...
    ungroup()
liana_constrained <- liana_wrap(seurat_object,
                                method = c('logfc','sca', 'connectome'),
                                resource = c('Consensus'),
                                cell.adj = cell.adj)
saveRDS(liana_constrained, file.path(liana_path, "testdata",
...
require(magrittr)
liana_res$natmi %>%
    rank_method("natmi", "specificity") %T>%
    saveRDS(file.path(liana_path, "testdata",
                      "output", "rank_spec_example.RDS")) %>%
    rank_method("natmi", "magnitude") %T>%
    saveRDS(file.path(liana_path, "testdata",
...
# liana heatmaps -----
saveRDS(heat_freq(readRDS(file.path(liana_path, "testdata",
                                    "output", "liana_cpdb.RDS")) %>%
...
liana_res <- readRDS(file.path(liana_path, "testdata",
                               "output", "liana_res.RDS"))
test <- liana_res$natmi %>%
    select(source, target, ligand.complex, receptor.complex)
test1 <- test %>%
    mutate(score = runif(n = nrow(.)))
test2 <- test %>%
    mutate(score = runif(n = nrow(.)))

context_df_dict <- list(test1=test1,
                        test2=test2)
saveRDS(context_df_dict, "inst/testdata/output/context_df_test.RDS")

decomp <- liana_tensor_c2c(context_df_dict = context_df_dict,
                           score_col = 'score',
                           rank = 5,  # set to None to estimate!
                           how='outer',  #  defines how the tensor is built
                           inplace = FALSE
)
decomp
saveRDS(decomp, "inst/testdata/output/decomp.RDS")

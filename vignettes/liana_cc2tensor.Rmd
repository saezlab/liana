...
saveRDS(context_df_dict, file = "~/Repos/liana2/data/pbmcs_lrs.RDS")
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=16, fig.height=12}
...
```{r, echo=FALSE, fig.width=11, fig.height=8, message=FALSE, warning=FALSE}
# Get all boxplots
all_facts_boxes <- plot_context_boxplot(factors$contexts, paired=TRUE)
...
`Factor.6`.
```{r ccpairs, echo=FALSE, fig.width=11, fig.height=8, message=FALSE, warning=FALSE}
...
```{r, echo=FALSE, fig.width=9, fig.height=13, message=FALSE, warning=FALSE}
...
```
When we focus on factor 6, we see that the source/sender cell types have almost
...
```
On the contrary, we see that few target cell types are driving the communication
patterns observed in factor 6, also visible in the Overview plot.

...
Yet, we kept the original method pipelines can be called via the `call_*` functions.
...
Here, we call Connectome, SCA, and Squidpy for the sake of computational speed,
but any of c("sca", "natmi", "logfc", "cellchat", "squidpy", "connectome") can be run by simply
...
liana_test <- liana_wrap(testdata,
                         method = c('squidpy', 'connectome', "sca"),
                         resource = c('OmniPath'))
```
Further, by default `sca` and `connectome` scores are obtained via their `liana` re-implementations, while Squidpy is called externally.

...
# each resource by target category. Then we create a summary dataframe to
...
add_omnipath <- omnipath_intercell %>% 
  group_by(category_intercell_target) %>% summarise(n = n()) %>%
...
data_w_omnipath <- omnipath_intercell %>% 
  ...
  group_by(sources, category_intercell_target) %>%
  summarise(n = n()) %>%
  ungroup %>%
  rbind(add_omnipath)%>%

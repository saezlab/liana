...
sce@metadata$tensor_res <-
  liana_tensor_c2c(context_df_dict = sce@metadata$liana_res,
                   score_col = 'LRscore',
                   rank = 8,  # set to None to estimate!
                   how='inner'  #  defines how the tensor is built
                   )
```

Optimal rank estimation can be a RAM demanding process, here the number of
ranks was pre-determined.

The `how` parameter plays is essential in how we treat missing cell types and
interactions. In this scenario, we use `inner`, which will only decompose CCC
patterns across cell identities & interactions shared between *all* samples. Alternatively,
one could change it to 'outer' CCC paterns will be decomposed across *any*
cell identities and interactions in the dataset.
...
all_facts_boxes <- plot_context_boxplot(factors$contexts,
                                        test="t_test", # applicable only to two groups
...
plot_contexts_heat(factors$contexts)
